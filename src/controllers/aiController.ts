  ...
  processMessage = asyncHandler(async (req: Request, res: Response) => {
    ...
    if (toolAction) {
      ...
      if (tool && args) {
        ...
        // Merge location data and access token if available in context
        ...
        // Always include access token with Bearer prefix (empty if not available)
        mergedArgs.access_token = authToken ? `Bearer ${authToken}` : '';
        ...
        if (toolResult.success) {
          ...
          // Check for errors even if toolResult.success is true
          if (data && data.isError === true && Array.isArray(data.content) && data.content.length > 0) {
            aiResponse.text = data.content[0].text; // Extract the specific error message
            logger.debug('Set responseText from toolResult.data.content (isError true)', { text: aiResponse.text });
          } else {
            ...
            } else if (toolResult.data && toolResult.data.status === 'clarification_needed') {
              aiResponse.text = toolResult.data.message;
              logger.debug('Set clarification message from tool', { text: aiResponse.text });
          ...
          // Propagate the exact error message from MCP server
          aiResponse.text = toolResult.error || 'Unknown error from MCP tool';
          logger.debug('Set tool failure message from MCP error', { text: aiResponse.text });
        }
      }
    }
